...
public class AudioManager : MonoBehaviour
{
    ...
    [SerializeField] private AudioEventChannelSO _SFXEventChannel = default;
    [Tooltip("The SoundManager listens to this event, fired by objects in any scene, to play Music")]
    [SerializeField] private AudioEventChannelSO _musicEventChannel = default;
    ...
    [SerializeField] SoundEmitter musicSoundEmitter;
    ...
    /// </summary>
    public void PlayAudioCue(AudioSO audioCue, AudioConfigurationSO settings, Vector3 position = default)
    {
        ...
        for (int i = 0; i < nOfClips; i++)
        {
            ...
            if (settings.OutputAudioMixerGroup == musicMixerGroup && musicSoundEmitter != null)
            {
                soundEmitter = musicSoundEmitter;
            }
            else
            {
                soundEmitter = _pool.Request();
                if (settings.OutputAudioMixerGroup == musicMixerGroup)
                {
    ...
    {
        // Update Sound Volume
        _musicVolume = Save.Get(SaveConstants.Music, 1f);
        _sfxVolume = Save.Get(SaveConstants.Sfx, 1f);
        SetGroupVolume("MasterVolume", _masterVolume);
        SetGroupVolume("MusicVolume", _musicVolume);
        SetGroupVolume("SFXVolume", _sfxVolume);
    }
