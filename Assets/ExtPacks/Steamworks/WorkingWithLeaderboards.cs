...
public class WorkingWithLeaderboards : MonoBehaviour
{
    ...
    private HeathenEngineering.SteamApi.PlayerServices.SteamworksLeaderboardData leaderboardToUpdate;
    [HideInInspector] public int currentlevelHighScore;


    // Scripts
    private SteamworksLeaderboardManager _SteamworksLeaderboardManager;


    private void Awake()
    {
        // Get Leaderboard Manager
        _SteamworksLeaderboardManager = GetComponent<SteamworksLeaderboardManager>();
    }
    ...
    /// </summary>
    public void SetThePlayerScoreAndOverwriteOldScore(string currentlyLevel, float score)
    {
        //Sets the player's score overwriting any existing value
        //feetTraveled.UploadScore((int)score, Steamworks.ELeaderboardUploadScoreMethod.k_ELeaderboardUploadScoreMethodForceUpdate);


        // Get Leaderboard to update name
        string leaderboardToUpdateName = SceneManager.GetActiveScene().name + " Leaderboard";

        // Get Leaderboard
        leaderboardToUpdate = _SteamworksLeaderboardManager.GetLeaderboard(leaderboardToUpdateName);

        // Update Player score in leaderbaoard
        leaderboardToUpdate.UploadScore((int)score, Steamworks.ELeaderboardUploadScoreMethod.k_ELeaderboardUploadScoreMethodForceUpdate);


        //Debug.Log(leaderboardToUpdateName + " Updated Player Score");
    }
    ...
    /// </summary>
    public void ReadThePlayersCurrentScoreFromCashe(string currentlyLevel)
    {
        //The UserEntry is a Nullable so we first test to see if it has a value at all ... if so report its data
        // if (feetTraveled.UserEntry.HasValue) { Debug.Log("The player's current rank is: " + feetTraveled.UserEntry.Value.m_nGlobalRank + ", the player's current score is: " + feetTraveled.UserEntry.Value.m_nScore + " the player has " + feetTraveled.UserEntry.Value.m_cDetails + " detail entries"); }
        // else { Debug.LogWarning("The player doesn't have a cahsed entry on this board yet."); }
        ...
        // Get player highcore from leaderboard
        if (leaderboardToUpdate.UserEntry.HasValue) { currentlevelHighScore = leaderboardToUpdate.UserEntry.Value.m_nScore; }       
    }
